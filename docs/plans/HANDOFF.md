...
- **Don't re-try:** GPU memory snapshot (`enable_gpu_snapshot`) segfaults on restore (exit 139) on this config; embedding batching is bit-identical but *slower* (padding waste); **pinning the Embedder to the cluster torch 2.1+cu121+cudnn8 stack** (still 80.6 ms/seq on A10G — it's the GPU, not the stack; also forces numpy<2); **`cpu=4` on the Embedder** (already 17 vCPUs, tokenize+h2d only ~0.9 ms/seq) — all ruled out with evidence.
